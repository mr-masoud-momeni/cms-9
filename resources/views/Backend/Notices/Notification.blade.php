...
@section('content')
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <form method="post" action="{{route('notification.store')}}" id="NotificationForm">
                                {!! csrf_field() !!}
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="slug" class="form-control" id="title" placeholder="نامک را وارد کنید..." value="{{old('slug')}}">
                                </div>
                                <div class="form-group">
                                    <label for="title">متن</label>
                                    <textarea name="body" class="form-control"></textarea>
                                </div>
                                <button type="submit"  class="btn btn-success">ایجاد پیغام </button>
                            </form>
                        </div>
                ...
                <div class="modal-dialog margin-top-60">
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title">ویرایش پیغام همگانی</h4>
                        </div>
                        <form method="post" action="" class="NotificationFormEdit">
                            <div class="modal-body">
                                {!! csrf_field() !!}
                                {{method_field('patch')}}
                                <div class="form-group">
                                    <label for="NotificationTitle">عنوان پیغام</label>
                                    <input type="text" name="title" class="form-control" id="NotificationTitle" placeholder="عنوان را وارد کنید..." value="">
                                    <input type="hidden" name="id" class="form-control" id="NotificationID" value="">
                                </div>
                                <div class="form-group">
                                    <label for="NotificationLabel">نامک</label>
                                    <input type="text" name="slug" class="form-control" id="NotificationSlug" placeholder="نامک را وارد کنید..." value="">
                                </div>
                                <div class="form-group">
                                    <label for="title">متن</label>
                                    <textarea name="body" id="NotificationBody" class="form-control"></textarea>
                                </div>
                            </div>
    ...
    <script>
        $(document).ready(function () {
            $('.NotificationFormEdit').submit(function (event) {
                event.preventDefault();
                var $this = $(this);
                var url = $this.attr('action');
                $.ajax({
    ...
    <script>
        $(document).on("click", ".editajax", function () {
            ...
            var NotificationSlug = $(this).data('slug');
            var NotificationBody = $(this).data('body');
            $(".modal-body #NotificationTitle").val( NotificationTitle );
            $(".modal-body #NotificationID").val( NotificationID );
            $(".modal-body #NotificationSlug").val( NotificationSlug );
            $(".modal-body #NotificationBody").val( NotificationBody );
            $(".NotificationFormEdit").attr("action", "{{url("/admin/notification/")}}/" + NotificationID);
        });
    </script>
    <script>
        $(document).ready(function () {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if($.isEmptyObject(data.error)){
                            $('#ajaxadd').prepend('<tr class="item'+data.success.id+'" style="background-color:#ccf3a5; "><td>'+data.success.title+'</td><td>'+data.success.slug+'</td><td>'+data.success.body+'</td><td><a class="editajax"  data-toggle="modal" data-target="#myModal" data-id="'+data.success.id+'" data-title="'+data.success.title+'" data-slug="'+data.success.slug+'" data-body="'+data.success.body+'" ><i class="fa fa-2x fa-pencil-square-o" aria-hidden="true"></i></a></td><td><a class="deleteAjax" data-toggle="modal" data-target="#DeleteModal" data-id="'+data.success.id+'" ><i class="fa fa-2x fa-trash-o" aria-hidden="true"></i></a></td></tr>');
                            var d = 1000;
                            for(var i=83; i<=100; i=i+0.1){
                                d  += 10;
                                (function(ii,dd){
